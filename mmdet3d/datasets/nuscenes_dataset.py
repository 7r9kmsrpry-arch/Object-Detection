...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        Returns:
            dict: 前処理パイプラインに渡されるデータ情報。主に以下のキーを含む。
                ...
                - pts_filename (str): 点群ファイル名
                - sweeps (list[dict]): sweep情報
                - timestamp (float): サンプルのタイムスタンプ
                - img_filename (str, optional): 画像ファイル名
                - lidar2img (list[np.ndarray], optional): LiDAR座標から各カメラ画像座標への変換行列
        ...
        input_dict = dict(
            ...
            pts_filename=info['lidar_path'],   # LiDAR点群ファイルのパス
            sweeps=info['sweeps'],             # 過去sweepの情報
            timestamp=info['timestamp'] / 1e6, # タイムスタンプ(マイクロ秒→秒に変換)
        )
        # ann_infos(ego基準のアノテーション情報)があれば追加
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv': 
                ...
                for cam_type, cam_info in info['cams'].items():
                    ...
                    lidar2cam_rt[3, :3] = -lidar2cam_t # lidar→cameraなので、-にしてる
                    # 内部パラメータを4×4に拡張
                    intrinsic = cam_info['cam_intrinsic']
                    viewpad = np.eye(4)
                    viewpad[:intrinsic.shape[0], :intrinsic.
                            shape[1]] = intrinsic
                    # LiDAR座標の点を画像座標へ投影するための行列を作成
                ...
                if '4d' in self.img_info_prototype: # ここに入る
                    # 隣接フレームの情報を追加
    ...
    def get_adj_info(self, info, index):
        ...
        stereo モード時は、通常の隣接フレームに加えて、`multi_adj_frame_id_cfg[1]` で指定された追加フレームも対象に含める。
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        """推論結果を nuScenes の標準提出形式に変換する。

        Args:
            results (list[dict]): データセット全サンプル分の推論結果のリスト。
                各要素には、少なくとも以下が含まれることを想定する。
                - 'boxes_3d': 3Dバウンディングボックス
                - 'scores_3d': 各ボックスの信頼度
                - 'labels_3d': 各ボックスのクラスラベル
            jsonfile_prefix (str, optional): 出力する json ファイルの保存先ディレクトリ
                またはファイル名の接頭辞。None の場合は未指定。
                実際の出力ファイル名は、この配下に
                'results_nusc.json' として作成される。

        Returns:
            str: 出力した json ファイルのパス。
        """
