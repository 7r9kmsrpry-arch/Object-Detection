...
class CenterPoint(MVXTwoStageDetector):
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        # # debug
        # if self.frame <= 1:
        #     np.savetxt(f"{self.frame}_x_origin.txt", x[0].cpu().numpy().reshape(-1))
        ...
        #     np.savetxt(f"{self.frame}_cls_out_origin.txt", cls_out[0].cpu().numpy())
        #     np.savetxt(f"{self.frame}_bbox_out_origin.txt", bbox_out[0].cpu().numpy())
