...
class BaseInstance3DBoxes(object):
    ...
    def to(self, device):
        """現在の3Dバウンディングボックスを、指定したデバイス上のオブジェクトに変換する。

        Args:
            device (str | torch.device): 移動先のデバイス名。
                例: 'cpu', 'cuda:0'

        Returns:
            BaseInstance3DBoxes: 指定したデバイスへ移動した、新しい3Dバウンディングボックスオブジェクト。
                戻り値は元と同じクラス型を維持し、内部のテンソルのみが指定デバイスへ移動する。
        """
