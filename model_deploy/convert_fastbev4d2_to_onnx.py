...
import torch.nn as nn
class FastBEV4DWrapper(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model

    def forward(self, imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda):
        inputs = [imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda]
        cls_out, bbox_out = self.model(inputs)
...
def main():
    ...
    for i, data in enumerate(data_loader):
        inputs = [d.cuda() for d in data['img_inputs'][0]]
        imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda, _ = model.prepare_inputs(inputs)
        wrapper = FastBEV4DWrapper(model).cuda().eval()

        out_path_4d2 = os.path.join(args.work_dir, "fastbev_4d2.onnx")
        if os.path.exists(out_path_4d2):
            os.remove(out_path_4d2)

        with torch.no_grad():
            cls_out, bbox_out = wrapper(imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda)
        ...
        torch.onnx.export(
            ...
            out_path_4d2,
            opset_version=16,
            input_names=["imgs","sensor2keyegos","ego2globals","intrins","post_rots","post_trans","bda"],
    ...
    onnx_model = onnx.load(out_path_4d2)
    onnx.checker.check_model(onnx_model)
    print("ONNX Model Correct")
