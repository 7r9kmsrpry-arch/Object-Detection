...
def main():
    ...
    for i, data in enumerate(data_loader):
        # data["img_inputs"] : list, len:1
        # data["img_inputs"][0] : [imgとかが7要素入ってる] 
        # if i == 0:
        #     # accelerateの有無で結果が一致するかを確認
        ...
        # 入力情報の作成
        #img = data['img_inputs'][0][0][0].cuda() # 先頭バッチの画像を取得 [N,C,H,W]←4dの場合、[12, 3, 256, 704]
        
        #　現在フレームと過去フレームの情報を分ける
        ...
        # 画像特徴量の作成
        x, _ = model.image_encoder(img_curr)

        # 座標変換情報の作成
        input_data = [x, sensor2keyegos_curr, ego2globals_curr, intrins_curr, post_rots_curr, post_trans_curr, bda]
        _, coors_img, coors_depth = model.img_view_transformer.get_fastray_input(input_data) # 事前計算済みの座標変換情報
        ...
        with torch.no_grad():
            torch.onnx.export(
                model,
                (
                    img_curr[0].float().contiguous(), # [Ncam, C, H, W]
                    coors_img.contiguous(),        # [num_voxel]
                    coors_depth.contiguous(),
                ),
                args.work_dir + model_prefix + '.onnx', # onnxファイルの保存先
                opset_version=11,
    ...
    # ONNXモデルの検証(FastBEVTRT)
    onnx_model = onnx.load(args.work_dir + model_prefix + '.onnx')
    try:
        onnx.checker.check_model(onnx_model)
    except Exception:
        print('ONNX Model(FastBEVTRT) Incorrect')
    else:
        print('ONNX Model(FastBEVTRT) Correct')
    # 計算グラフの簡素化
